...
# -*- coding: utf-8 -*-
"""
Analyse av daglige strømpriser per måned (2015-2024) med visualiseringer og beregninger.
...
OUTPUT_DIR = "/Users/pathujahvimalasenan/Desktop/Master/Grafer/Måned/"
...
FONT_SIZE = 22
Y_LIMITS_YEAR = {
    2015: (0, 40), 2016: (0, 85), 2017: (0, 50), 2018: (0, 95), 2019: (0, 80),
    2020: (0, 45), 2021: (0, 400), 2022: (0, 700), 2023: (0, 200), 2024: (0, 260)
}
Y_LIMITS_MONTH = {"jan": (0, 225), "jul": (0, 225), "okt": (0, 240)}
PLOT_PROPS = {
...
def normalize_data(df_no_2022):
    """Normaliserer daglige priser til gjennomsnitt = 1 per måned og år."""
    daily_data = df_no_2022.groupby(["År", "Måned", "Dag_i_måneden"])["SPOTNO1(Øre/kWh)"].mean().reset_index()
    daily_data["Normalisert_Pris"] = np.nan
    ...
    for year in YEARS:
        ...
        for month in range(1, 13):
            ...
            prices = daily_data.loc[mask, "SPOTNO1(Øre/kWh)"].values
            if len(prices) > 0 and np.mean(prices) != 0:
                daily_data.loc[mask, "Normalisert_Pris"] = prices / np.mean(prices)
    return daily_data


def plot_boxplot(data, output_path):
    """Lager boxplott for normaliserte dagspriser per måned."""
    ...
    for month in range(1, 13):
        ...
        data_per_day = [month_data[month_data["Dag_i_måneden"] == d]["Normalisert_Pris"].dropna().values 
                        for d in range(1, 32)]
    ...
    fig.suptitle("Boxplott av normaliserte dagspriser (2015-2024, uten 2022)", fontsize=FONT_SIZE)
    ...
    pd.DataFrame(tabell_data).to_csv(os.path.join(OUTPUT_DIR, "median_std_per_dag_og_måned.csv"), index=False)
    return median_std_data


def plot_median_with_std(median_std_data, output_path):
    """Lager medianlinjepl channeled for each month with ±1 standard deviation."""
    fig, axes = plt.subplots(4, 3, figsize=(15, 20), sharex=True, sharey=True)
    axes = axes.flatten()
    ...
    fig.suptitle("Syntetisk Månedsprofil (2015-2024, uten 2022)", fontsize=FONT_SIZE)
    fig.text(0.5, 0.04, "Dag i måneden", ha="center", fontsize=FONT_SIZE)
    fig.text(0.02, 0.5, "Normalisert spotpris", va="center", rotation="vertical", fontsize=FONT_SIZE)
...
def plot_yearly_comparison(df, median_std_data, monthly_avg):
    ...
    for year in YEARS:
        ...
        for month in range(1, 13):
            year_month_data = df[(df["År"] == year) & (df["Måned"] == month)].groupby("Dag_i_måneden")["SPOTNO1(Øre/kWh)"].mean().reset_index()
            median_per_day = median_std_data[MONTHS[month-1]]["Median"]
            std_per_day = median_std_data[MONTHS[month-1]]["Std"]
            avg_price = monthly_avg.get((year, month), 0)
            ...
            synthetic = [val * avg_price if not np.isnan(val) else np.nan for val in median_per_day]
            synthetic_std = [std * avg_price if not np.isnan(std) else np.nan for std in std_per_day]
            ...
            if not year_month_data.empty:
                axes[month-1].plot(year_month_data["Dag_i_måneden"], year_month_data["SPOTNO1(Øre/kWh)"], 
...
def plot_selected_months(df, median_std_data, monthly_avg):
    ...
    for month_name, month_num in SELECTED_MONTHS.items():
        ...
        for idx, year in enumerate(YEARS):
            year_month_data = df[(df["År"] == year) & (df["Måned"] == month_num)].groupby("Dag_i_måneden")["SPOTNO1(Øre/kWh)"].mean().reset_index()
            median_per_day = median_std_data[month_name]["Median"]
            std_per_day = median_std_data[month_name]["Std"]
            avg_price = monthly_avg.get((year, month_num), 0)
            ...
            if not year_month_data.empty:
                axes[idx].plot(year_month_data["Dag_i_måneden"], year_month_data["SPOTNO1(Øre/kWh)"], 
                               label="Rådata", color="blue", linewidth=2, marker="o")
...
def calculate_jsd(df, median_std_data, monthly_avg):
    ...
    for year in YEARS:
        for month in range(1, 13):
            year_month_data = df[(df["År"] == year) & (df["Måned"] == month)].groupby("Dag_i_måneden")["SPOTNO1(Øre/kWh)"].mean().reindex(range(1, 32)).values
            median_per_day = median_std_data[MONTHS[month-1]]["Median"]
            avg_price = monthly_avg.get((year, month), 0)
...
def main():
    ...
    # Beregn månedlig gjennomsnittspris
    monthly_avg = df.groupby(["År", "Måned"])["SPOTNO1(Øre/kWh)"].mean().to_dict()

    # Normaliser data
    daily_data = normalize_data(df_no_2022)

    # Generer plott
    median_std_data = plot_boxplot(daily_data, os.path.join(OUTPUT_DIR, "boxplot_gjennomsnitt_1_uten_2022.png"))
    ...
    print(f"Tabeller: median_std_per_dag_og_måned.csv, jsd_verdier_per_år.csv, jsd_verdier_jan_jul_okt_per_år.csv, rmse_mae_mbe_per_år_måned.csv")
