...
def load_data(file_path):
    ...
    if df["Time"].isna().any():
        raise ValueError("Ugyldige datetime-verdier i 'Hour'-kolonnen. Kontroller formatet i kolonnen 'Hour'.")
    if "SPOTNO1(Øre/kWh)" not in df.columns:
        raise ValueError("Kolonne 'SPOTNO1(Øre/kWh)' mangler i datafilen.")
    return df
...
def plot_comparison(df, median_per_month, std_per_month, avg_prices):
    ...
    for period, years in PERIODS.items():
        ...
        for idx, year in enumerate(years):
            ...
            # Plott rådata, konverter Series til NumPy arrays for kompatibilitet
            if not year_data.empty:
                x_data = year_data["Måned"].to_numpy()
                y_data = year_data["SPOTNO1(Øre/kWh)"].to_numpy()
                axes[idx].plot(
                    x_data,
                    y_data,
                    label="Rådata",
...
def main():
    ...
    for period in PERIODS:
        print(f"   - {period}: sammenligning_syntetisk_rådata_{period}.png")
    print(f"JSD-plott: jsd_gjennomsnitt_per_år.png")
    print(f"Tabeller: median_std_per_måned.csv, jsd_verdier_per_år.csv, rmse_mae_mbe_per_år.csv")
