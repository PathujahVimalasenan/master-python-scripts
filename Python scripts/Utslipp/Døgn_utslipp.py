...
# -*- coding: utf-8 -*-
"""
Analyse av timebaserte CO2-utslippsintensiteter per døgn (2015-2024) med visualiseringer og beregninger.
...
OUTPUT_DIR = "/Users/pathujahvimalasenan/Desktop/Master/Data/CO2/grafer/Døgn/"
...
FONT_SIZE = 22
Y_LIMITS_YEAR = {
    2015: (0, 430), 2016: (0, 450), 2017: (0, 430), 2018: (0, 430), 2019: (0, 360),
    2020: (0, 370), 2021: (0, 390), 2022: (0, 410), 2023: (0, 330), 2024: (0, 360)
}
Y_LIMITS_MONTH = {"jan": (0, 420), "jul": (0, 450), "okt": (0, 500)}
PLOT_PROPS = {
...
def normalize_data(df):
    """Normaliserer utslippsintensiteter til gjennomsnitt = 1 per måned og år."""
    hourly_data = df.groupby(["År", "Måned", "Time_i_døgnet"])["Utslippsintensitet"].mean().reset_index()
    hourly_data["Normalisert_Utslipp"] = np.nan

    for year in YEARS:
        for month in range(1, 13):
            mask = (hourly_data["År"] == year) & (hourly_data["Måned"] == month)
            emissions = hourly_data.loc[mask, "Utslippsintensitet"].values
            if len(emissions) > 0 and np.mean(emissions) != 0:
                hourly_data.loc[mask, "Normalisert_Utslipp"] = emissions / np.mean(emissions)
    return hourly_data


def plot_boxplot(hourly_data, output_path):
    """Lager boxplott for normaliserte utslippsintensiteter per måned."""
    ...
    for month in range(1, 13):
        ...
        data_per_hour = [month_data[month_data["Time_i_døgnet"] == h]["Normalisert_Utslipp"].dropna().values 
                         for h in range(24)]
    ...
    fig.suptitle("Boxplott av normaliserte utslippsintensiteter (2015-2024)", fontsize=FONT_SIZE)
    ...
    pd.DataFrame(tabell_data).to_csv(os.path.join(OUTPUT_DIR, "median_std_per_time_og_måned_utslipp.csv"), index=False)
...
def plot_median_with_std(median_std_data, output_path):
    ...
    fig.suptitle("Syntetisk Døgnprofil (2015-2024)", fontsize=FONT_SIZE)
    fig.text(0.5, 0.04, "Time i døgnet", ha="center", fontsize=FONT_SIZE)
    fig.text(0.02, 0.5, "Normalisert utslippsintensitet", va="center", rotation="vertical", fontsize=FONT_SIZE)
...
def plot_yearly_comparison(df, median_std_data, daily_avg):
    ...
    for year in YEARS:
        ...
        for month in range(1, 13):
            year_month_data = df[(df["År"] == year) & (df["Måned"] == month)].groupby("Time_i_døgnet")["Utslippsintensitet"].mean().reset_index()
            median_per_hour = median_std_data[MONTHS[month-1]]["Median"]
            std_per_hour = median_std_data[MONTHS[month-1]]["Std"]
            avg_emission = daily_avg.get((year, month), 0)
            ...
            synthetic = [val * avg_emission if not np.isnan(val) else np.nan for val in median_per_hour]
            synthetic_std = [std * avg_emission if not np.isnan(std) else np.nan for std in std_per_hour]
            ...
            if not year_month_data.empty:
                axes[month-1].plot(year_month_data["Time_i_døgnet"].to_numpy(), 
                                   year_month_data["Utslippsintensitet"].to_numpy(), 
...
def plot_selected_months(df, median_std_data, daily_avg):
    ...
    for month_name, month_num in SELECTED_MONTHS.items():
        ...
        for idx, year in enumerate(YEARS):
            year_month_data = df[(df["År"] == year) & (df["Måned"] == month_num)].groupby("Time_i_døgnet")["Utslippsintensitet"].mean().reset_index()
            median_per_hour = median_std_data[month_name]["Median"]
            std_per_hour = median_std_data[month_name]["Std"]
            avg_emission = daily_avg.get((year, month_num), 0)
            ...
            if not year_month_data.empty:
                axes[idx].plot(year_month_data["Time_i_døgnet"].to_numpy(), 
                               year_month_data["Utslippsintensitet"].to_numpy(), 
                               label="Rådata", color="blue", linewidth=2, marker="o")
...
def calculate_jsd(df, median_std_data, daily_avg):
    ...
    for year in YEARS:
        for month in range(1, 13):
            year_month_data = df[(df["År"] == year) & (df["Måned"] == month)].groupby("Time_i_døgnet")["Utslippsintensitet"].mean().reindex(range(24)).values
            median_per_hour = median_std_data[MONTHS[month-1]]["Median"]
            avg_emission = daily_avg.get((year, month), 0)
...
def main():
    ...
    # Beregn daglig gjennomsnittsutslipp per måned
    daily_avg = df.groupby(["År", "Måned", "Dag_i_måneden"])["Utslippsintensitet"].mean().groupby(["År", "Måned"]).mean().to_dict()

    # Normaliser data
    hourly_data = normalize_data(df)

    # Generer plott
    median_std_data = plot_boxplot(hourly_data, os.path.join(OUTPUT_DIR, "boxplot_gjennomsnitt_1_utslipp.png"))
    ...
    print(f"Tabeller: median_std_per_time_og_måned_utslipp.csv, jsd_verdier_per_år_utslipp.csv, jsd_verdier_jan_jul_okt_per_år_utslipp.csv, rmse_mae_mbe_per_år_måned_utslipp.csv")
